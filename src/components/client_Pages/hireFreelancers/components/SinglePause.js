...
const ContactModal = (props) => {
  const {
    ...
    first_name,
    last_name,
  } = props.pause.freelancer.user_profile
  const { email } = props.pause.freelancer
  return (
          ...
          <div className="row">
            <div className="col-md-4 pb-4">
              <Avatar
                alt="Profile"
                src={
                  pause.freelancer.user_profile.user_profile_image == null
                    ? DefaultProfile
                    : `http://localhost:3000/${pause.freelancer.user_profile.user_profile_image}`
                }
            ...
            <div className="col-md-6 pb-4 pt-0 pt-md-3">
              <h4 className="text-color pb-2 h2">
                {pause.freelancer.user_profile.full_name == " "
                  ? `${pause.freelancer.user_profile.first_name}  ${pause.freelancer.user_profile.last_name}`
                  : `${pause.freelancer.user_profile.full_name}`}
              </h4>
              <p className="pb-2">
                <strong>Status: </strong>
                <span className="text-info">
                  {pause.freelancer.is_active ? "Active" : "Offline"}{" "}
                </span>{" "}
                <strong> Hours limit: </strong>
                <span className="text-info">
                  {
                    pause.client_project.client_availibility
                      .client_project_duration["4"]
                  }
                </span>
              </p>
            </div>
          ...
          <h4>
            <small>
              ...
              <strong className="text-color">
                {" "}
                {pause.freelancer.user_profile.user_skills}{" "}
              </strong>
              at a rate of ${pause.freelancer.user_profile.minimum_hourly_wage}{" "}
              per hour
            </small>
          </h4>
        </CardFooter>
